...
export default function ComparePlansView({
  ...
  return (
    <div className="flex min-h-0 flex-col gap-5">
            ...
            <div
              ...
              className={`flex flex-col rounded-xl border p-4 text-left ${
                isCurrent
                  ...
                  : "border-(--yoco-border-subtle)"
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <span className="flex items-center gap-1.5">
                  <span className={`inline-block h-2.5 w-2.5 shrink-0 rounded-full ${PLAN_COLORS[tier].dotClass}`} />
                  <span className="text-sm font-semibold">{planLabel(tier)}</span>
                </span>
                {isCurrent ? (
                  <span className="rounded-full bg-(--yoco-primary) px-2 py-0.5 text-[10px] font-semibold text-white">
                    Current
                  </span>
                ) : null}
              </div>
              ...
              <p className="mt-1 text-xs text-(--yoco-text-muted)">{count} modules included</p>
        ...
        {(() => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <span className="flex items-center gap-1.5">
                  <span className={`inline-block h-2.5 w-2.5 shrink-0 rounded-full ${PLAN_COLORS.CUSTOM.dotClass}`} />
              ...
              <p className="mt-2 text-sm font-semibold leading-snug">Build a Custom plan</p>
              <p className="mt-1 text-xs text-(--yoco-text-muted)">Choose individual modules.</p>
              <div className="mt-auto pt-3">
                <Button
      ...
      <div className="overflow-x-auto rounded-xl border border-(--yoco-border-subtle)">
        <table className="w-full min-w-[36rem] border-collapse text-sm">
          <thead>
            <tr className="border-b border-(--yoco-border-subtle) bg-(--yoco-surface-muted)/50">
              <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-(--yoco-text-muted)">
                Module
              </th>
              {STANDARD_TIERS.map((tier) => (
                <th
          ...
          <tbody>
            {MODULE_CATALOG.map((mod) => (
              <tr key={mod.key} className="border-b border-(--yoco-border-subtle) last:border-b-0">
